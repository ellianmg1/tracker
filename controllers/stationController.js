...
//};
    ...
    async.parallel({
        ...
        // Successful, so render.
        res.render('station_detail', { title: 'Station Detail', user: req.user, station:  results.station });
    });
...
exports.station_create_get = function(req, res, next) {
    ...
    res.render('station_form', {title: 'Add Station', user: req.user});
};
...
exports.station_create_post =  [
    ...
    body('state', 'State is required').isLength({ min: 1 }).trim(),    
    ...
    (req, res, next) => {
        ...
        if (!errors.isEmpty()) {
            ...
            res.render('station_form', { title: 'Add Station', user: req.user, station: station, errors: errors.array()});
        return;
        }
        else {
...
exports.station_update_get = function(req, res, next) {
    ...
    Station.findById(req.params.id, function(err, station) {
        ...
        // Success
        res.render('station_form', { title:'Update Station', user: req.user, station: station});
    }
)
};
...
exports.station_update_post = [ 
    ...
    (req, res, next) => {
        ...
        if (!errors.isEmpty()) {
            // There are errors. Render the form again with sanitized values/error messages.
            res.render('station_form', { title: 'Update Station', user: req.user, station: station, errors: errors.array()});
        return;
        }
        else {
